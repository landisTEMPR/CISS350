...
int max(Node *proot)
{
    ...
    {
        std::cout << "starting root: " << proot->key_ << '\n';
        int left_max = max(proot->left_);
        std::cout << "left max: " << left_max << '\n';
        int right_max = max(proot->right_);
        std::cout << "right max: " << right_max << '\n';
        return max(proot->key_, left_max, right_max);
    }
}
...
int main()
{
    ...
    std::cout << *find(n11, 12) << '\n';

    std::cout << *leftmost(n11) << '\n';
    // Clean up memory
